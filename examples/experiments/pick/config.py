...
class PickEnvConfig(DefaultEnvConfig):
    ...
    IMAGE_CROP = {
        ...
        "world": lambda img: img[160:300, 200:360],

    }
    IMAGE_SIZE = (128, 128)

    ACTION_SCALE = (0.015, 0.1)
    TRANSLATION_LIMIT = np.array([0.3, 0.3, 0.3])
    WORKSPACE_LOW = np.array([0.35, -0.16, 0.00])
...
class TrainConfig(DefaultTrainingConfig):
    ...
    max_traj_length = PickEnvConfig.MAX_EPISODE_LENGTH
    buffer_period = 1000
    ...
    def get_environment(self, fake_env=False, save_video=False, classifier=False):
        from experiments.pick.wrapper import (
        ...
        env = PickEnv(config=PickEnvConfig(), fake_env=fake_env)
