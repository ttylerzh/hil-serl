...
class InsertPlugEnvConfig(DefaultEnvConfig):
    ...
    ACTION_SCALE = (0.005, 0.1)
    TRANSLATION_LIMIT = np.array([0.05, 0.05, 0.05])
    WORKSPACE_LOW = np.array([0.3, -0.2, 0.02])
    WORKSPACE_HIGH = np.array([0.6, 0.1, 0.31])
...
class TrainConfig(DefaultTrainingConfig):
    ...
    max_traj_length = InsertPlugEnvConfig.MAX_EPISODE_LENGTH
    buffer_period = 1000
    ...
    def get_environment(self, fake_env=False, save_video=False, classifier=False):
        ...
        env = InsertPlugEnv(config=InsertPlugEnvConfig(), fake_env=fake_env)
